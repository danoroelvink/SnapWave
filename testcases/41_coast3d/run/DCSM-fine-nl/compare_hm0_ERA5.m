clear all;close all;
load Coast3D_validationdata
casename='dcsm-fine-nl'
runid='coast3d_v3_g7';
fname=[casename,'\snapwave_',runid,'_his.nc'];
s2=sqrt(2);
...
for ind=1:8
    if Hindex(ind)~=0&~strcmp(names(ind,:),'7a ')
        ...
        [S(i)]=skill0([tm,hsm],[tm,hsc]);
        pointname{i}=names(ind,:);
...
figure(2)
export_fig(['Coast3D_',casename,'_Hm0_timeseries_comparison_',runid,'.png'])
figure(3)
export_fig(['Coast3D_',casename,'_Hm0_scatterplot_',runid,'.png'])

fi=fopen(['Coast3D_',casename,'_stats_Hm0_',runid,'.txt'],'w');
